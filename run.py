...
def send_email(app, to_email, subject, body):
    ...
    try:
        ...
        print(f"Email sending failed to {to_email} ({subject}): {e}")
        return False

# --- GEMINI API INTEGRATION & HELPERS ---
GEMINI_API_KEY = "AIzaSyBgjS79oR8LlW1-LR2IuBLndIqzuvjRSPE" # Hardcoded for local use
...
def is_email_valid(email):
    # A more robust regex for email validation. Requires at least one character before the last dot in the domain
...
def generate_rejection_feedback(reasons):
    ...
    prompt = f"""
    As a compassionate and professional HR manager, you need to draft a rejection feedback for a candidate.
    The candidate was rejected for the following reasons: {', '.join(reasons)}.
    Write a single, well-structured, professional, and encouraging feedback paragraph. 
    The feedback should offer constructive insight into areas for growth, without being overly negative or inventing new reasons for rejection.
    Start with a gracious opening, like "Thank you for taking the time to interview with us." 
    Keep the message concise and focused on how the candidate can develop.
    """
    try:
        ...
        feedback = "Thank you for your interest. After careful consideration, we have decided not to move forward with your application at this time. "
        if reasons:
            feedback += f"While we will not be moving forward at this time, we encourage you to continue developing your skills, particularly in areas such as: {', '.join(reasons)}. We wish you the best in your career endeavors."
        return feedback
    ...
    if request.method == "POST":
        ...
        for email in invited_emails:
            if is_email_valid(email):
                ...
                register_link = url_for('register', role='candidate', _external=True)
                subject = "Interview Invitation - AI Resume Screener"
                body = f"""
                    Hello Candidate,

                    You have been invited to an interview titled '{title}' on {interview_date} at {interview_time}.
                    Description: {description}

                    Please register or log in to the AI Resume Screener to view more details and apply:
                    {register_link}
                    ...
                    The HR Team
                    """
                print(f"DEBUG: Scheduling email to: {email}, Subject: {subject}")
...
@app.route("/hr/interview/<int:interview_id>/delete_application/<candidate_username>", methods=["POST"])
@login_required(role='hr')
def delete_application(interview_id, candidate_username):
    db = sqlite3.connect(DATABASE)
    cursor = db.cursor()
    try:
        # Verify the HR user owns this interview (optional, but good for security)
        cursor.execute("SELECT id FROM interviews WHERE id = ? AND hr_username = ?", (interview_id, session['username']))
        if not cursor.fetchone():
            flash("Interview not found or you do not have permission to modify it.", "error")
            return redirect(url_for('interview_details', interview_id=interview_id))

        # Get resume filename before deleting the application record
        ...
        if application_data:
            ...
            cursor.execute("DELETE FROM applications WHERE interview_id = ? AND candidate_username = ?", (interview_id, candidate_username))
            db.commit()

            # Delete the associated resume file
            resume_path = os.path.join(app.config['UPLOAD_FOLDER'], str(interview_id), resume_filename)
            if os.path.exists(resume_path):
                os.remove(resume_path)
                print(f"Deleted resume file: {resume_path}")
            flash(f"Application for {candidate_username} deleted successfully.", "success")

            # After deleting an application, reset results_published status for this interview
            # to allow the candidate to re-apply if needed.
        ...
        print(f"Error deleting application for {candidate_username} in interview {interview_id}: {e}")
    finally:
        db.close()
    return redirect(url_for('interview_details', interview_id=interview_id))
...
def publish_results(interview_id):
    ...
    cursor.execute("SELECT title FROM interviews WHERE id = ?", (interview_id,))
    interview_data = cursor.fetchone()
    interview_title = interview_data[0] if interview_data else f"Interview (ID: {interview_id})"

    if action == 'publish':
        cursor.execute("UPDATE interviews SET results_published = 1 WHERE id = ?", (interview_id,))
        db.commit()

        # Send emails to all candidates about their results
        cursor.execute("SELECT candidate_username, status, final_feedback FROM applications WHERE interview_id = ?", (interview_id,))
        all_applications = cursor.fetchall()

        for candidate_username, status, final_feedback in all_applications:
            cursor.execute("SELECT email FROM users WHERE username = ?", (candidate_username,))
            candidate_email = cursor.fetchone()
            if candidate_email:
                ...
                if status == 'Accepted':
                    body = f"""
                        ...
                        Congratulations! We are pleased to inform you that you have been accepted for the interview titled '{interview_title}'.
                    ...
                    # Rejection feedback is already generated and stored in final_feedback
                    body = f"""
                        Hello {candidate_username},

                        Thank you for your interest in the interview titled '{interview_title}'.
                        After careful consideration, we regret to inform you that we will not be moving forward with your application at this time.

                        Feedback from the HR team:
                        {final_feedback or 'No specific feedback was provided at this time.'}
                    ...
                    # For waiting list candidates, a generic message can be sent for now.
                    # Their specific rejection emails are handled in the 'reject_waiting_list' action.
                    body = f"""
                        Hello {candidate_username},

                        Thank you for your interest in the interview titled '{interview_title}'.
                        Your application is currently on a waiting list. We will notify you of any updates as soon as possible.

                        Best regards,
                        The HR Team
                        """
                
                if body: # Only send email if a body was constructed
                    print(f"DEBUG: Publishing results email to: {candidate_email[0]}, Subject: {subject}")
                    send_email(app, candidate_email[0], subject, body)
        ...
        # Send email to waiting list candidates
        for candidate, raw_feedback_json in waiting_list:
            cursor.execute("SELECT email FROM users WHERE username = ?", (candidate,))
            candidate_email = cursor.fetchone()
            if candidate_email:
                final_feedback = ""
                if raw_feedback_json:
                    try:
                        parsed_data = json.loads(raw_feedback_json)
                        reasons = parsed_data.get('rejection_reasons') or parsed_data.get('reasons')
                        if reasons and isinstance(reasons, list):
                            final_feedback = generate_rejection_feedback(reasons)
                        else:
                            final_feedback = generate_rejection_feedback(['The position has been filled.'])
                    except json.JSONDecodeError:
                        final_feedback = generate_rejection_feedback(['The position has been filled.'])
                else:
                    final_feedback = generate_rejection_feedback(['The position has been filled.'])

                subject = f"Interview Results for {interview_title}"
                body = f"""
                    Hello {candidate},

                    Thank you for your patience regarding your application for the interview titled '{interview_title}'.
                    We regret to inform you that the position has been filled, and we will not be moving forward with your application at this time.
                    ...
                    The HR Team
                    """
                print(f"DEBUG: Rejecting waiting list email to: {candidate_email[0]}, Subject: {subject}")
                send_email(app, candidate_email[0], subject, body)
...
if __name__ == "__main__":
    ...
    app.run(debug=True, host='0.0.0.0') # Hardcoding debug=True for local use
